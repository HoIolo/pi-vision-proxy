...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    label: "识图",
    description:
      "识别/处理一张本地图片(调用独立的多模态识图模型, 不消耗主模型多模态能力)。当主模型只支持文本、需要查看图片内容时调用, 例如用户消息中提到的图片文件、工具生成或发现的截图等。注意: 用户消息中直接附带的图片已被自动识别并作为描述文本提供, 无需对它们重复调用本工具。参数 path 为本地图片文件路径(支持 png/jpg/jpeg/gif/webp/bmp); 如有具体需求(提取配色、翻译图中文字、读取数据、描述布局、判断内容等)请通过 prompt 参数传入, 识图模型会按你的要求处理, 效果等同原生多模态模型看图。",
